...
def semihemisphere(lat,long):
    if (lat >= -90 and lat <= 0) and (long >= -180 and long <= 0):
        return ("Southwest")
    if (lat >= -90 and lat <= 0) and (long > 0 and long <= 180):
        return ("Southeast")
    if (lat > 0 and lat <= 90) and (long >= -180 and long <= 0):
        return ("Northwest")
    if (lat > 0 and lat <= 90) and (long > 0 and long <= 180):
        return ("Northeast")
